...
// ================================================================
//
// Basic evaluation function using only piece/square tables, originally
// created and tuned by  Ronald Friederich for his engine Rofchade.
// it has became a de-facto standard for programmers who want to jump
// to improving search straight away.
//
// For external description, see:
// https://www.chessprogramming.org/PeSTO%27s_Evaluation_Function

// We keep material values for reference and in init()
// we merge them with pst score for speed
...
// This is what eval function reads
var mgTable [2][6][64]int
var egTable [2][6][64]int

// Merge material and pst scores; calculate inverted pst tables
// for black pieces.
func init() {
	for pt := P; pt <= K; pt++ {
...
func evaluatePesto(p *Pos) int {
	...
	// Sum the piece/square scores for all the pieces,
	// separately midgame and endgame. Also, calculate
	// game phase from non-pawn material on both sides.
	for side := White; side <= Black; side++ {
		for pt := P; pt <= K; pt++ {
			...
			for pieces != 0 {
				...
				mg[side] += mgTable[side][pt][sq]
				eg[side] += egTable[side][pt][sq]
				phase += gamePhase[pt]
			}
		}
	}
