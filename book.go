...
var guideBook PolyglotBook
var mainBook PolyglotBook

// initializer for both books
func initBooks(guideBookName, mainBookName string) {
	initGuideBook(guideBookName)
	initMainBook(mainBookName)
}
...
func polyglotKey(p *Pos) uint64 {
	...
	if p.epSquare != NO_SQ /*&& polyglotEpCapturePossible(p)*/ {
		key ^= PG[772+fileOf(p.epSquare)]
	}

	return key
}

func (book *PolyglotBook) open(path string) bool {
	book.close()
	book.name = path

	data, err := os.ReadFile(path)
	if err != nil {
		fmt.Printf("info string reading book file %q: failure: %v\n", path, err)
		return false
	}

	if len(data) == 0 || len(data)%16 != 0 {
		fmt.Printf("info string reading book file %q: invalid Polyglot file size\n", path)
		return false
	}

	count := len(data) / 16
	book.entries = make([]PolyglotEntry, count)

	for i := 0; i < count; i++ {
		offset := i * 16

		book.entries[i] = PolyglotEntry{
			key:    binary.BigEndian.Uint64(data[offset : offset+8]),
			move:   binary.BigEndian.Uint16(data[offset+8 : offset+10]),
			weight: binary.BigEndian.Uint16(data[offset+10 : offset+12]),
			learn:  binary.BigEndian.Uint32(data[offset+12 : offset+16]),
		}
	}

	// Polyglot books should already be sorted, but sorting makes lookup safe
	// even with a non-conforming book.
...
func (book *PolyglotBook) close() {
	book.entries = nil
}

func (book *PolyglotBook) isOpen() bool {
	return len(book.entries) != 0
}

func (book *PolyglotBook) findFirst(key uint64) int {
	index := sort.Search(len(book.entries), func(i int) bool {
		return book.entries[i].key >= key
	})

	if index == len(book.entries) || book.entries[index].key != key {
...
func (book *PolyglotBook) isInfrequent(weight, maxWeight int) bool {
	if maxWeight > 2 && weight < 2 {
		return true
	}

	return weight < maxWeight*book.filter/100
}

func (book *PolyglotBook) getMove(p *Pos) int {
	if !book.isOpen() {
		return 0
	}
	...
	var choices []BookMove
	maxWeight := 0
	weightSum := 0

	for i := first; i < len(book.entries); i++ {
		entry := book.entries[i]
		if entry.key != key {
			break
		}

		move := polyglotMoveToInternal(p, entry.move)
		if move == 0 {
			fmt.Printf(
				"info string ignoring invalid Polyglot move %04x\n",
				entry.move,
			)
			continue
		}
		...
		choices = append(choices, BookMove{
			move:   move,
			weight: weight,
		})
	...
	for _, choice := range choices {
		...
		accumulatedWeight += choice.weight

		// Weighted reservoir sampling.
		if rand.Intn(accumulatedWeight) < choice.weight {
...
func polyglotMoveToInternal(p *Pos, encoded uint16) int {
	toFile := int(encoded & 7)
	toRank := int((encoded >> 3) & 7)
	fromFile := int((encoded >> 6) & 7)
	fromRank := int((encoded >> 9) & 7)
	promotion := int((encoded >> 12) & 7)
	...
	moveString := moveToStr((to << 6) | from)
